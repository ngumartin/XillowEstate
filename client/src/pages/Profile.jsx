  ...
  return (
    <div className="p-3 max-w-lg mx-auto">
      ...
      <div className="flex justify-between mt-5">
        <span onClick={handleDeleteUser} className="text-red-700 cursor-pointer border p-3 rounded-lg">Delete Account</span>
        <span onClick={handleSignOut} className="text-red-700 cursor-pointer border p-3 rounded-lg">Sign Out</span>
      </div>

      <p className="text-red-700 mt-5">{error ? error : ''}</p>
      <p className="text-green-700 mt-5">{updateSuccess ? 'Profile updated successfully!' : ''}</p>
      <button onClick={handleShowListings} className='text-green-700 w-full'>Show Listings</button>
      <p className='red-green-700 mt-5'>{showListingsError ? 'Error showing listings' : '' }</p>
      ...
      {userListings && 
        ...
        userListings.map((listing) => (
          <div 
            key={listing._id} 
            className='border rounded-lg p-3 my-5 flex justify-between items-center gap-4'
          >
